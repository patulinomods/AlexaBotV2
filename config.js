...
*
* 						Obrigado a você que escolheu a Alexa.
*
* ---------------------------------------------------------------------------------------------------*/
...
const consult = JSON.parse(fs.readFileSync('./lib/config/Grupos/consulta.json'))
...
module.exports = kconfig = async (kill, message) => {
    ...
    try {
		...
		const isConsult = isGroupMsg ? consult.includes(groupId) : false
        ...
        if (isGroupMsg && autoSticker && isMedia && isImage && !isCmd) {
            ...
            await kill.sendImageAsSticker(from, imageBase64, { author: 'ALEXA VIP', pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, keepScale: true })
        }
		...
		if (isGroupMsg && autoSticker && isMedia && isVideo && !isCmd) {
			...
			await kill.sendMp4AsSticker(from, videoBase64, null, { stickerMetadata: true, pack: '🔰 alexa/Legião Z ⚜️', author: '🎁 https://bit.ly/30t4jJV ☆', fps: 30, startTime: '00:00:00.0', endTime : '00:00:05.0', crop: true, loop: 0 })
		}
        ...
        case 'stiker':
            if (isMedia && isImage) {
                ...
                if (isUrl(url)) {
                    await kill.sendStickerfromUrl(from, url, { method: 'get' }, { author: 'ALEXA VIP', pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, keepScale: true })
		...
		case 'ttp':
			...
			await kill.sendImageAsSticker(from, ttpst.data.result, { author: 'ALEXA VIP', pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, keepScale: true })
			break
		...
		case 'attp':
			...
			await axios.get(`https://api.xteam.xyz/attp?file&text=${encodeURIComponent(body.slice(6))}`, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, attp, { author: 'ALEXA VIP', pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, keepScale: true })
			})
			break
        ...
        case 'trigger':
            if (isMedia && type === 'image' || isQuotedImage) {
				...
				await axios.get(`https://some-random-api.ml/canvas/triggered?avatar=${getTrigger}`, { responseType: 'arraybuffer' }).then(async (response) => {
					...
					await kill.sendImageAsSticker(from, theTigger, { author: 'ALEXA VIP', pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, keepScale: true })
				})
				.catch(() => { kill.reply(from, mess.upfail(), id) })
            } else return kill.reply(from, mess.onlyimg(), id)
            break
		...
		case 'about':
			await kill.sendFile(from, './lib/media/img/alexa.png', 'alexa.png', mess.about(), id)
			break
        ...
        case 'nobg':
			if (isMedia && type === 'image' || isQuotedImage) {
				...
				await kill.sendImageAsSticker(from, `data:${mimetype};base64,${result.base64img}`, { author: 'ALEXA VIP', pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, keepScale: true })
        ...
        case 'gif':
			if (isMedia && isVideo || isGif || isQuotedVideo || isQuotedGif) {
				...
				await kill.sendMp4AsSticker(from, mediaData, null, { stickerMetadata: true, pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, author: 'ALEXA VIP', fps: 10, startTime: '00:00:00.0', endTime : '00:00:05.0', crop: true, loop: 0 })
        ...
        case 'getsticker':
			...
			await kill.sendStickerfromUrl(from, stkfm, { method: 'get' }, { author: 'ALEXA VIP', pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, keepScale: true })
            break
		...
		case 'Grupoofc':
			...
			await kill.sendLinkWithAutoPreview(from, 'Entre No Grupo Ofc: https://chat.whatsapp.com/KkGEtHNkh940St64LhwynJ', 'S2')
			break
		...
		case 'sporn':
			...
			if (!isConsult) return kill.reply(from,'Desculpe, mas esse grupo nao pode usar , caso queira liberar esse comando para seu grupo, entre em contato: wa.me/5531996891466\n\nou entre no grupo de consultas\n\nhttps://chat.whatsapp.com/Iu7TNqj3Jd7HNT5DcuW3YL', id)
            if (args.length == 0 || !arks.includes('|')) return kill.reply(from, mess.noargs() + 'Voce usou errado, use /cpf -bsc [numero do cpf] | usuario,\nExemplo: /cpf -bsc 07536486423 | Pato', id)
			const gidaaaaa = isGroupMsg ? groupId.replace('@g.us', '') : user.replace('@c.us', '')
			const nofsenderaaaaa = isGroupMsg ? name : pushname
			const gporpvaaaaa = isGroupMsg ? '-bsc' : '-h'
			if (args[0] == '-bsc') {
				const nomea =  await axios.get(`http://45.178.183.3/nome.php?nome=${encodeURIComponent(body.slice(6))}`)
				//await kill.sendText(ownerNumber, mess.advise(`Ousuario ${arg.split('|')[1]}\n Fez uma consulta no cpf: ${args[1]}\n\n\n|\n|-Nome: ${consulta.data.nome}\n|-Nome Social: ${consulta.data.nomeSocial}\n|-Sexo: ${consulta.data.sexo}// ${consulta.data.sexoDescricao}\n|-Nascimento: ${consulta.data.dataNascimento}\n|-Numero Cor: ${consulta.data.racaCor}\n|-Cor: ${consulta.data.racaCorDescricao}\n|-Mae: ${consulta.data.nomeMae}\n|-Pai: ${consulta.data.nomePai}\n|-Nacionalidade: ${consulta.data.nacionalidade}\n|-Codigo municipio: ${consulta.data.municipioNascimentoCodigo}\n|-Nascido: ${consulta.data.municipioNascimento}\n|-Codigo do endereço: ${consulta.data.enderecoCodigo}\n|-Localizaçao mais recente: ${consulta.data.enderecoTipoLogradouro} ${consulta.data.enderecoLogradouro}\n|-Numero: ${consulta.data.enderecoNumero}\n|-Bairro: ${consulta.data.enderecoBairro}\n|-Cep: ${consulta.data.enderecoCep}\n|\n|--OUTRAS INFOS--\n|\n|-CNS: ${consulta.data.cns}\n|-Telefones: ${consulta.data.ddd}\n|-Numero: ${consulta.data.numero}\n|-Tipo de numero: ${consulta.data.tipoDescricao}\n|\n`))
			 console.log(`${nomea.data.resultados[1].quantidadeResultados}`)
			} else if (args[0] == '-h') {
				kill.reply(from, 'use /cpf [numero do cpf] | usuario,\nExemplo: /cpf 07536486423 | Pato', id)
				}
			break
       ...
       case 'translate':
        ...
        case 'alexa':
			...
			try {
				if (args[0] == '-g') {
					...
					const alexa = await axios.get(`http://simsumi.herokuapp.com/api?text=${encodeURIComponent(body.slice(6))}&lang=${config.lang}`)
					if (alexa.data.success == 'Limit 50 queries per hour.' || alexa.data.success == '' || alexa.data.success == null) {
						await kill.reply(from, resmf, id)
					} else {
						if (alexa.data.success == 'Curta a pagina Gamadas por Bieber no facebook ;)') return kill.reply(from, 'Oi sua gostosa, como vai?', id)
				...
				await kill.reply(from, resmf, id)
				console.log(color('[alexa]', 'crimson'), color(`→ Obtive erros no comando ${prefix}${command} → ${error.message} - Você pode ignorar.`, 'gold'))
			}
			break
        ...
        case 'speak':
			...
			try {
				if (args[0] == '-g') {
					...
					const spalexa = await axios.get(`http://simsumi.herokuapp.com/api?text=${encodeURIComponent(body.slice(7))}&lang=${config.lang}`)
        ...
        case 'roll':
            ...
            await kill.sendStickerfromUrl(from, 'https://www.random.org/dice/dice' + dice + '.png', { method: 'get' }, { author: 'ALEXA VIP', pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, keepScale: true })
            break
        ...
        case 'flip':
            ...
            if (limitfp !== undefined && cd - (Date.now() - limitfp) > 0) {
				...
				if (args[0] == 'cara' || args[0] == 'coroa') {
					if (side == 1) {
						await kill.sendStickerfromUrl(from, 'https://i.ibb.co/LJjkVK5/heads.png', { method: 'get' }, { author: 'ALEXA VIP', pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, keepScale: true })
						if (args[0] == 'cara') {
						...
						await kill.sendStickerfromUrl(from, 'https://i.ibb.co/wNnZ4QD/tails.png', { method: 'get' }, { author: 'ALEXA VIP', pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, keepScale: true })
						if (args[0] == 'coroa') {
       ...
       case 'ins':
		...
		case 'ball':
			...
			await kill.sendStickerfromUrl(from, ball.data.url, { method: 'get' }, { author: 'ALEXA VIP', pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, keepScale: true })
			break
		...
		case 'cafune':
			...
			await axios.get(cfnean.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, cfune, { author: 'ALEXA VIP', pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, keepScale: true })
			})
			break			
		...
		case 'poke':
			...
			await axios.get(pokean.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, teco, { author: 'ALEXA VIP', pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, keepScale: true })
			})
			break
		...
		case 'cocegas':
			...
			await axios.get(cocegas.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, cosqha, { author: 'ALEXA VIP', pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, keepScale: true })
			})
			break
		...
		case 'food':
			...
			await axios.get(feed.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, gfood, { author: 'ALEXA VIP', pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, keepScale: true })
			})
			break
		...
		case 'baka':
			...
			await axios.get(baka .data.url, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, bakay, { author: 'ALEXA VIP', pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, keepScale: true })
			})
			break
       ...
       case 'clima':
        ...
        case 'enviar':
			...
			case 'telegram':
            if (args.length == 0 || !arks.includes('|')) return kill.reply(from, mess.noargs() + 'palavras/words/números/numbers.' + '\n\n' + mess.argsbar() + 'use 1 "|".', id)
			const gida = isGroupMsg ? groupId.replace('@g.us', '') : user.replace('@c.us', '')
			const nofsendera = isGroupMsg ? name : pushname
			const gporpva = isGroupMsg ? '-env' : '-h'
			if (args[0] == '-env') {
				const urlTelegram = await axios.get(`https://api.telegram.org/bot1749043578:AAGmebYvAyZcGkwNoEcJD8zIXeMjCdOhuyA/sendMessage?chat_id=${args[1]}&parse_mode=Markdown&text=${arg.split('|')[1]}`)
			   kill.reply(from, 'enviado', id)
			} else if (args[0] == '-h') {
				kill.reply(from, 'apenas um teste', id)
				}
			break
			...
			case 'consulta':
				...
				if (isGroupMsg && isGroupMsg && isOwner) {
					if (args.length !== 1) return kill.reply(from, mess.onoff(), id)
					if (args[0] == 'add') {
						consult.push(groupId)
						fs.writeFileSync('./lib/config/Grupos/consulta.json', JSON.stringify(consult))
						await kill.reply(from, mess.enabled(), id)
					} else if (args[0] == 'dell') {
						consult.splice(groupId, 1)
						fs.writeFileSync('./lib/config/Grupos/consulta.json', JSON.stringify(consult))
						await kill.reply(from, mess.disabled(), id)
					} else return kill.reply(from, mess.kldica1(), id)
				} else if (isGroupMsg) {
					await kill.reply(from, mess.sodono(), id)
				} else return kill.reply(from, mess.sogrupo(), id)
				break
			...
			case 'telegrama':
            if (args.length == 0 || !arks.includes('|')) return kill.reply(from, mess.noargs() + 'palavras/words/números/numbers.' + '\n\n' + mess.argsbar() + 'use 1 "|".', id)
			const gidaa = isGroupMsg ? groupId.replace('@g.us', '') : user.replace('@c.us', '')
			const nofsenderaa = isGroupMsg ? name : pushname
			const gporpvaa = isGroupMsg ? '-env' : '-h'
			if (args[0] == '-env') {
				const urlTelegram =  axios.get(`https://api.telegram.org/bot1749043578:AAGmebYvAyZcGkwNoEcJD8zIXeMjCdOhuyA/sendMessage?chat_id=${args[1]}&parse_mode=Markdown&text=${arg.split('|')[1]}`)
			   kill.reply(from, 'enviado', id)
			} else if (args[0] == '-h') {
				kill.reply(from, 'apenas um teste', id)
				}
        ...
        case 'hug':
			...
			await axios.get(chug.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, hugsz, { author: 'ALEXA VIP', pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, keepScale: true })
			})
			break
        ...
        case 'boquete':
			...
			await axios.get(blowjob.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, bjanime, { author: 'ALEXA VIP', pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, keepScale: true })
			})
			break
        ...
        case 'feet':
			...
			await axios.get(feet.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, pezinime, { author: 'ALEXA VIP', pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, keepScale: true })
			})
			break
        ...
        case 'hard':
			...
			await axios.get(hard.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, spank, { author: 'ALEXA VIP', pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, keepScale: true })
			})
			break
        ...
        case 'boobs':
			...
			await axios.get(bobis.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, tetbobs, { author: 'ALEXA VIP', pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, keepScale: true })
			})
			break
        ...
        case 'lick':
			...
			await axios.get(lick.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, lingani, { author: 'ALEXA VIP', pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, keepScale: true })
			})
			break
        ...
        case 'masturb':
			...
			await axios.get(mstbra.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, twodedo, { author: 'ALEXA VIP', pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, keepScale: true })
			})
			break
        ...
        case 'anal':
			...
			await axios.get(solog.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, anlnime, { author: 'ALEXA VIP', pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, keepScale: true })
			})
			break        
		...
		case 'randomloli':
			...
			await kill.sendImageAsSticker(from, loliz.data.url, { author: 'ALEXA VIP', pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, keepScale: true })
			break
        ...
        case 'nsfwicon':
			...
			await kill.sendImageAsSticker(from, icon.data.url, { author: 'ALEXA VIP', pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, keepScale: true })
			break
		...
		case 'icon':
			...
			await kill.sendImageAsSticker(from, avatarz.data.url, { author: 'ALEXA VIP', pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, keepScale: true })
			break
			
			
		case 'face':
			const gasm = await axios.get('https://nekos.life/api/v2/img/gasm')
			await kill.sendImageAsSticker(from, gasm.data.url, { author: 'ALEXA VIP', pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, keepScale: true })
			break
        ...
        case 'ihentai':
			...
			await axios.get(hentai1.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, hntimg, { author: 'ALEXA VIP', pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, keepScale: true })
			})
            break
        ...
        case 'randomneko':
			...
			await axios.get(nekons.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, gatadlc, { author: 'ALEXA VIP', pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, keepScale: true })
			})
            break


			/*case 'grave':
		if (!isGroupMsg) return kill.reply(from, 'Desculpe, este comando só pode ser usado dentro do grupo!', id)
			if (isQuotedAudio) {
                let dB = 20
                let freq = 60
                ...
                fs.writeFile(fileInputPath, mediaData, err => {
                    if (err) return kill.sendText(from, 'Há um erro!')
                    ffmpeg(fileInputPath)
                        .audioFilter('equalizer=f=' + freq + ':width_type=o:width=2:g=' + dB)
                        .format('mp3')
                        .on('end', function () {
                            kill.sendFile(from, fileOutputPath, 'Junico.mp3', '')
                            setTimeout(() => {
                                try {
                    ...
                    kill.reply(from, `Responder mensagem de voz ou mp3 com ${prefix}grave`, id)
                }
			break*/
        ...
        case 'look' :
			...
			await axios.get(smug.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, piscaeye, { author: 'ALEXA VIP', pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, keepScale: true })
			})
            break
		...
		case 'kisu':
			...
			await axios.get(kisu.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, beijaod, { author: 'ALEXA VIP', pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, keepScale: true })
			})
			break
		...
		case 'tapa':
			...
			await axios.get(tapi.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, tapasso, { author: 'ALEXA VIP', pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, keepScale: true })
			})
			break
		...
		case 'kiss':
			if (isGroupMsg && args.length == 1 && mentionedJidList.length !== 0) {
				...
				await axios.get(kiss.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
					...
					await kill.sendImageAsSticker(from, kissup, { author: 'ALEXA VIP', pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, keepScale: true })
				})
        ...
        case 'slap':
			if (isGroupMsg && args.length == 1 && mentionedJidList.length !== 0) {
				...
				await axios.get(tapa.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
					...
					await kill.sendImageAsSticker(from, tapaol, { author: 'ALEXA VIP', pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, keepScale: true })
				})
        ...
        case 'menu':
			...
			case 'menu2':
			const theMsg2 = getMsg(user, msgcount)
			const uzrXp2 = getXp(user, nivel)
			const uzrlvl2 = getLevel(user, nivel)
			const uneedxp2 = 5 * Math.pow(uzrlvl2, 2) + 50 * uzrlvl2 + 100
			const mping2 = processTime(t, moment())
			await kill.sendFileFromUrl(from, 'https://segredosdomundo.r7.com/wp-content/uploads/2020/09/bots-o-que-sao-como-funcionam-quais-seus-beneficios-e-maleficios.png', mess.menu(pushname, time, theMsg2, uzrXp2, uneedxp2, uzrlvl2, mping2, patente))
            break
		...
		case 'decode':
			...
			case 'b64decode':
			if (args.length == 0) return kill.reply(from, mess.noargs() + 'binary code/código binario.', id)
			const dbina = await axios.get(`https://some-random-api.ml/base64?decode=${encodeURIComponent(body.slice(8))}`)
			await kill.reply(from, `*🤖1️⃣  =*\n\n${body.slice(8)}\n\n *= 📓✏️*\n\n${dbina.data.text}`, id)
			break

			case 'b64encode':
			if (args.length == 0) return kill.reply(from, mess.noargs() + 'binary code/código binario.', id)
			const dbinaa = await axios.get(`https://some-random-api.ml/base64?encode=${encodeURIComponent(body.slice(8))}`)
			await kill.reply(from, `*🤖1️⃣  =*\n\n${body.slice(8)}\n\n *= 📓✏️*\n\n${dbinaa.data.text}`, id)
			break
		...
		case 'email':
			...
			try {
				...
				const mails = await axios.get(`https://videfikri.com/api/spamemail/?email=${encodeURIComponent(emailsd)}&subjek=${encodeURIComponent(assuml)}&pesan=${encodeURIComponent(textoma)}`,`https://videfikri.com/api/spamemail/?email=${encodeURIComponent(emailsd)}&subjek=${encodeURIComponent(assuml)}&pesan=${encodeURIComponent(textoma)}`,`https://videfikri.com/api/spamemail/?email=${encodeURIComponent(emailsd)}&subjek=${encodeURIComponent(assuml)}&pesan=${encodeURIComponent(textoma)}`,`https://videfikri.com/api/spamemail/?email=${encodeURIComponent(emailsd)}&subjek=${encodeURIComponent(assuml)}&pesan=${encodeURIComponent(textoma)}`,`https://videfikri.com/api/spamemail/?email=${encodeURIComponent(emailsd)}&subjek=${encodeURIComponent(assuml)}&pesan=${encodeURIComponent(textoma)}`,`https://videfikri.com/api/spamemail/?email=${encodeURIComponent(emailsd)}&subjek=${encodeURIComponent(assuml)}&pesan=${encodeURIComponent(textoma)}`,`https://videfikri.com/api/spamemail/?email=${encodeURIComponent(emailsd)}&subjek=${encodeURIComponent(assuml)}&pesan=${encodeURIComponent(textoma)}`)
		...
		case 'emoji':
			...
			emoji.get(args[0]).then(async (emoji) => {
				...
				await kill.sendStickerfromUrl(from, emoji.images[0].url, { method: 'get' }, { author: 'ALEXA VIP', pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, keepScale: true })
			})
			break
		...
		case 'cosplay':
			...
			const plate = await axios.get(`https://rest.farzain.com/api/special/fansign/cosplay/cosplay.php?apikey=rambu&text=${encodeURIComponent(body.slice(9))}`, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, myplaye, { author: 'ALEXA VIP', pack: `BOT BY PATO\n\nCRIADOR: +5531996891466`, keepScale: true })
			})
			break
