...
const alexavs = require('./package.json')
...
const start = async (kill = new Client()) => {
	...
	if (alexavs.version !== getversion.data.version) { console.log(color('\n[UPDATE]', 'crimson'), color(`Uma nova versão da Alexa foi lançada [${getversion.data.version}], atualize para obter melhorias e correções! → ${alexavs.homepage}`, 'gold')) }
	console.log(color('\n[SUPORTE]', 'magenta'), color(`https://chat.whatsapp.com/H53MdwhtnRf7TGX1VJ2Jje | +55 18 99804-4132 | ${alexavs.bugs.url}\n`, 'blue'), color(`\n[Alexa ${alexavs.version}]`, 'magenta'), color('Estamos prontos para começar mestre!\n', 'green'))
